...
async def lifespan(app: FastAPI):
    ...
    async def scheduled_reboot_monitor():
        """Reboot the host daily at the configured time without interrupting motion."""
        ...
        while True:
            try:
                ...
                state.save()
                # The native service runs as root, so systemctl avoids an
                # interactive sudo prompt and works consistently under systemd.
...
class PlaylistRequest(BaseModel):
    ...
    run_mode: str = "single"
    shuffle: bool = False
    # When true, pause_time is a start-to-start cadence rather than a delay
    # added after the pattern finishes.
...
async def add_to_queue(request: dict):
    ...
    # Current playlist entries are THETA_RHO_DIR-joined paths; inserting the
    # bare filename causes the executor to skip the queued pattern.
